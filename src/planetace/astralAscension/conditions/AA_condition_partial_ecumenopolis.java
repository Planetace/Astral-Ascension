...
// Partial Ecumenopolis Condition
// Increases Hazard, increases Access, Market Income, Fleet Size, and Defence Combat Effectiveness.
public class AA_condition_partial_ecumenopolis extends BaseMarketConditionPlugin {
    public static float ACCESS_BONUS = 25f;
    public static float INCOME_MULTIPLIER = 1.1f;
    public static float FLEET_ADDITION = 25f;
    public static float GROUND_ADDITION = 1.25f;


    public void apply(String id) {
        super.apply(id);
        // Modify Market Aspects
        market.getAccessibilityMod().modifyFlat(id,ACCESS_BONUS/100f, "Partial Ecumenopolis");
        market.getIncomeMult().modifyMult(id,INCOME_MULTIPLIER, "Partial Ecumenopolis");
        // Modify Military Aspects
        market.getStats().getDynamic().getMod(Stats.COMBAT_FLEET_SIZE_MULT).modifyFlat(id, FLEET_ADDITION/100f, "Partial Ecumenopolis");
        market.getStats().getDynamic().getMod(Stats.GROUND_DEFENSES_MOD).modifyMult(id, GROUND_ADDITION,"Partial Ecumenopolis");
    }
